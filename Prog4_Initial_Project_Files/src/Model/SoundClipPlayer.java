...
public class SoundClipPlayer implements Runnable {
	...
	// Main loop for the Model.SoundClipPlayer thread
	// The thread will dequeue a Model.SoundClip from the
	// Model.SoundClipBlockingQueue and play it. If the queue
	// is empty, the thread will go to sleep until
	// there is a Model.SoundClip to retrieve from the queue.
	@Override
	public void run(){
		SoundClip sc = null;
		while (true) {
	...
	/**
	 * Plays the WAV Model.SoundClip given as argument
	 * @param sc - the Model.SoundClip to play
	 */
